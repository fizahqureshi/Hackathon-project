...
import { signInWithEmail, signInWithGoogle, createRecaptcha, sendPhoneVerification } from '../firebase/firebaseConfig';
...
export default function Login() {
	...
	return (
		<div className="auth-page">
			<motion.div className="auth-card" initial={{ y: -30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.45 }}>
				<div className="brand">
				...
				<form onSubmit={handleEmailLogin}>
					...
					<motion.button whileHover={{ scale: 1.02 }} className="btn" type="submit" disabled={loading}>
						{loading ? 'Signing in...' : 'Sign in'}
					</motion.button>
				</form>

				<div style={{margin:'12px 0', textAlign:'center'}} className="helper">or</div>

				<div className="social-row">
					<motion.div whileHover={{ y: -4 }} className="social" onClick={handleGoogle}>
						<FcGoogle size={20} />
						<span style={{marginLeft:8}}>Google</span>
					</motion.div>
					<motion.div whileHover={{ y: -4 }} className="social" onClick={() => {}}>
				...
				<div className="small-link">
					<span className="helper">Don't have an account? <Link to="/signup">Sign up</Link></span>
				</div>
			</motion.div>
		</div>
	);
